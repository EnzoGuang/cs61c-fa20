...
mapLoop:
    #mistake add t1, s0, x0      # load the address of the array of current node into t1
    lw t1, 0(s0)
    lw t2, 4(s0)        # load the size of the node's array into t2
    # mistake1 t1 increment by 4; add the next line
    slli t3, t0, 2
    # mistake2
    # add t1, t1, t0
    add t1, t1, t3      # offset the array address by the count
    lw a0, 0(t1)        # load the value at that address into a0
    #mistake, add the line
    add t4, t1, x0      # the jalr s1 will modified the value of t1
    jalr s1             # call the function on that value.
    #mistake the t1 has been modified
    # sw a0, 0(t1)
